...
# 이유알어걸
def print_5xn(str):
    chk_num = int(len(str) / 5)
    for i in range(chk_num + 1):
        print(str[i * 5: i * 5 + 5])

print_5xn("아이엠어보이유알어걸아이엠미씽유")
...
# 유알어
# 걸
def print_mxn(strings, cap):
    chk_num = int(len(strings) / cap)
    for i in range(chk_num + 1):
        print(strings[i * cap: i * cap + cap])
